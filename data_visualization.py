...
df_2 = pd.read_sql_query('SELECT KY_CD, OFNS_DESC,COUNT(*) as `num_crimes`'
                       ...
                       'ORDER BY -num_crimes '
                        'LIMIT 10 ', crime_data)
fig2 = py.iplot([Bar(x=df_2.OFNS_DESC, y=df_2.num_crimes)], filename='Top 10 Offense types with KY')
fig2.save('fig2.png')
...
df_5= pd.read_sql_query('SELECT CRM_ATPT_CPTD_CD,COUNT(*) as `num_crimes`'
                       ...
                       'ORDER BY -num_crimes ', crime_data)
fig5 = py.iplot([go.Pie(labels=df_5.CRM_ATPT_CPTD_CD, values=df_5.num_crimes)], filename='Most Common Crimes by CRM_ATPT_CPTD_CD')
fig5.save('fig5.png')
